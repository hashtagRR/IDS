# all classifdiers with Ordinal encoder
...
class IntrusionDetectionSystem:
    ...
    def encode_non_numerics(self):
        ...
        # Identify numerical columnsd e4mrt
        ...
        # Ordinal encode categorical columns for training and testing data
        encoder = OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1)
        
        # Encode each categorical column separately
        for column in non_numerical_columns:
            # Fit and transform the training data
            self.raw_train_data[column] = encoder.fit_transform(self.raw_train_data[[column]])

            # Transform the test data, handling unknown categories
            self.raw_test_data[column] = encoder.transform(self.raw_test_data[[column]])

            # Concatenate numerical columns, encoded categorical columns, and 'attack_cat' for training data
        self.train_data = pd.concat([self.raw_train_data[numerical_columns], self.raw_train_data[non_numerical_columns], self.raw_train_data['attack_cat']], axis=1)
        #print("Columns in self.train_data:\n", self.train_data.columns)

        # Concatenate numerical columns, encoded categorical columns, and 'attack_cat' for test data
        self.test_data = pd.concat([self.raw_test_data[numerical_columns], self.raw_test_data[non_numerical_columns], self.raw_test_data['attack_cat']], axis=1)

       
    def split_dataset(self):
        columns_to_drop = ['id', 'label','attack_cat', 'proto']

        # Remove 'Normal' traddic type
        self.train_data = self.train_data[self.train_data['attack_cat'] != 'Normal']
        self.test_data = self.test_data[self.test_data['attack_cat'] != 'Normal']
    ...
    def balance_dataset(self):
        ...
        '''save_path = "/home/azureuser/cloudfiles/code/users/2306143/processed.csv"
        balanced_data = pd.concat([self.x_train, pd.DataFrame({'label': self.y_train})], axis=1)
    ...
    def train_voting_classifier(self):
        #type = "Voting > RF, GB, XGB"
        clf_xgb = xgb.XGBClassifier(learning_rate=0.01, n_estimators=100, max_depth=10, subsample=10, colsample_bytree=10,
    objective='multi:softmax', num_class=9, eval_metric='logloss')
        clf_rf = RandomForestClassifier(random_state=1)
        #clf_gb = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=1, random_state=0)
        clf_v = VotingClassifier(estimators=[('xgb', clf_xgb), ('rf', clf_rf)], voting='soft')
